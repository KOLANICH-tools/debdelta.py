...
class SQL_queue(object):
    ...
    def queue_add_begin(self):
        assert self.sql_connection_add is None
        self.sql_connection_add = self._connect()
    def queue_add(
        self, priority, old_name, new_name, delta, forensic, other_info="", ctime=None
    ):
        if self.sql_connection_add is None:
            raise Exception(" should use queue_add_begin() before ")
        if ctime is None:
            ctime = int(time.time())
        with self.sql_connection_add as S:
            S.execute(
                "INSERT INTO deltas_queue VALUES (null, ?, ?, ?, ?, ?, ?, ?)",
    ...
    def queue_pop(self, id_=None):
        ...
        try:
            ...
            if id_ is None:
                cursor.execute(
                    "SELECT * FROM deltas_queue ORDER BY priority , ctime LIMIT 1 "
                )
            else:
                cursor.execute("SELECT * FROM deltas_queue WHERE id = ? ", (id_,))
            x = cursor.fetchone()
...
def html(db, W):
    ...
    for x in s.iterate():
        ...
        x = ["" if (j is None) else j for j in x]
