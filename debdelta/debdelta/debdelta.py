...
try:
    ...
    a = sys.exc_info[1]
    sys.stderr.write(
        'Could not initialize "gettext", translations will be unavailable\n' + str(a)
    )
    def _(x):
        return x
...
if sys.version_info.major == 2:
    ...
    if sys.getfilesystemencoding() != "UTF-8":
        logger.warning(
            "The filesystem encoding is %r, but this program assumes `UTF-8`."
    ...
    if sys.getfilesystemencoding() != "utf-8":
        ...
        hasattr(sys, "getfilesystemencodeerrors")
        and sys.getfilesystemencodeerrors() != "surrogateescape"
    ):
        logger.warning(
            "The filesystem encoding is `utf-8` with %r, but this program uses `surrogateescape`."
...
DEB_FORMAT_LIST = (
    ...
    "unzipped",
    "preunpacked",
)  # not yet implemented on patching side :  (,'piped')
    ...
    for o, v in opts:
        if o == "-v":
            ...
            if not os.path.isdir(DIR):
                sys.stderr.write(
                    _("Error: argument of --dir is not a directory:") + " " + DIR + "\n"
                )
                raise SystemExit(3)
            ...
            if not os.path.isdir(FORENSICDIR):
                sys.stderr.write(
                    _("Error: argument of --forensicdir is not a directory:")
                    + " "
                    + FORENSICDIR
                    + "\n"
                )
            ...
            if FORENSIC[:4] == "http":
                try:
                    ...
                    sys.stderr.write(
                        _(
                            'To use the http forensic, you must install the package "python-poster".'
                        )
                        + "\n"
                    )
                    raise SystemExit(3)
            if FORENSIC[:4] in ("mutt", "mail") and not os.path.exists("/usr/bin/mutt"):
                sys.stderr.write(
                    _('To use this forensic, you must install the package "mutt".')
                    + "\n"
                )
                raise SystemExit(3)
        elif o == "--alt":
            if not (os.path.isfile(v) or os.path.isdir(v)):
                sys.stderr.write(
                    _("Error: argument of --alt is not a directory or a regular file:")
                    + " "
                    + v
                    + "\n"
                )
            ...
            if not (os.path.isfile(v) or os.path.isdir(v)):
                sys.stderr.write(
                    _("Error: argument of --old is not a directory or a regular file:")
                    + " "
                    + v
                    + "\n"
                )
            ...
            GPG_SIGNING_KEY = v
            DO_GPG = True
        elif (o == "--accept-unsigned" or o == "-A") and action in (
            "delta-upgrade",
            "patch",
        ):
            DO_GPG = False
...
try:
    a = subprocess.Popen(
        b"grep bogomips /proc/cpuinfo", shell=True, stdout=subprocess.PIPE
    ).stdout.read()
    a = bytes2str(a)
...
def test_deb822(debian_deb822, shared_storage):
    ...
    I = debian_deb822.Packages.iter_paragraphs(
        sequence=open(F.name, "rb"), shared_storage=shared_storage
    )
    c = next(I)
...
def my_popen_read(cmd, **args):
    ...
    return subprocess.Popen(
        cmd, stdout=subprocess.PIPE, stdin=open(os.devnull), close_fds=True, **args
    ).stdout
...
class cache_sequence(object):
    ...
    def __init__(self, filename, cache_filename=None):
        ...
        """
        self.cache_filename = (
            os.path.splitext(filename)[0] + self.suffix
        ...
        self.cache = None
        self.broken = None
        self.exists = os.path.isfile(self.cache_filename) and os.path.getmtime(
            filename
        ) <= os.path.getmtime(self.cache_filename)
    def __iter__(self):
    ...
    def write(self, s):
        ...
        try:
            ...
            logger.exception(
                "Cannot write to cache file, deleting %r", self.cache_filename
            )
            self.close()
            if ACT:
                os.unlink(self.cache_filename)
...
class cache_same_dict(cache_sequence):
    ...
    def __next__(self):
        ...
        return dict(
            map(lambda x, y: (x, y), self.keys, n)
        )  # dict comprehension may be used instead
...
def apply_prepare_for_echo(shell, repres):
    ...
    p = subprocess.Popen(
        [shell], stdin=subprocess.PIPE, stdout=subprocess.PIPE, close_fds=True
    )  # , ** Popen_args)
...
def delta_base_name(pkg, old_ver, new_ver, arch, ext=".debdelta"):
    ...
    return (
        pkg
        + "_"
        + version_mangle(old_ver)
        + "_"
        + version_mangle(new_ver)
        + "_"
        + arch
        + ext
    )
...
class DebDeltaError(
    Exception
):  # should derive from (Exception):http://docs.python.org/dev/whatsnew/pep-352.html
...
class PopenPipe(object):
    def __init__(self, a, **dictargs):
        ...
        while "|" in a:
            ...
            a = a[l + 1 :]
            p = subprocess.Popen(
                args=a1, stdin=old_stdin, stdout=subprocess.PIPE, **dictargs
            )
            self.pros.append(p)
            old_stdin = p.stdout
        final_pro = subprocess.Popen(
            args=a, stdin=old_stdin, stdout=old_stdout, **dictargs
        )
        for p in self.pros:
...
def system(a, TD, saveargs=None, ignore_output=False, return_output=False):
    ...
    if ignore_output == False and (
        os.stat(temp_name)[ST_SIZE] > 0 or os.stat(temp_err_name)[ST_SIZE] > 0
    ):
        logger.debug(
            ' command "%s" returned %d and  produced output as follows' % (a, ret)
        )
    ...
    if ret == 0:
        ...
        try:
            ...
            s = (
                s
                + '\n    (there was an additional OSError "'
                + str(o)
                + '" when trying to save arguments)'
            )
        die(s)
...
def _info_patch_unzip_(TD):
    ...
    if os.path.exists(TD + "PATCH/patch.sh.gz"):
        ...
        if not os.path.exists("/usr/bin/unlzma"):
            raise DebDeltaError(
                'This patch needs lzma. Please install the Debian package "lzma".',
                retriable=True,
            )
        ...
        if not os.path.exists("/usr/bin/unxz"):
            raise DebDeltaError(
                'This patch needs xz. Please install the Debian package "xz-utils".',
                retriable=True,
            )
...
def get_info_fast(delta):
    ...
    if s[:4] != "info":
        ...
        if VERBOSE > 1:
            logger.warning(
                "  (Warning, cannot get info from old style: " + delta + " )"
            )
        return None
    # parse ar segment
    ...
    if s[-2:] != "`\n":
        logger.warning(
            "(Warning, cannot get info from  " + delta + " , format not known)"
        )
        return None
    l = int(s[-12:-2])
...
def patch_check_tmp_space(params, olddeb):
    ...
    if free < instsize:
        return _(
            "not enough disk space (%(free)dkB) in directory %(dir)s for applying delta (needs %(size)dkB)"
        ...
        for l in sorted(L):
            if bytar:
                ...
                if os.path.exists(divert):
                    fullname, mode, tartype, uid, gid, uname, gname, data = stat_to_tar(
                        divert
                    )
                else:
                    fullname, mode, tartype, uid, gid, uname, gname, data = (
                        "",
                        0,
                        "?",
                        0,
            ...
            if (
                name in (".", "/", "./", "/.") and tartype == tarfile.DIRTYPE
            ):  # skip root
                continue
            if uname is None:
                uname = str(uid)
...
def forensic_send(f, forensic=FORENSIC):
    ...
    if forensic is False or forensic == "False" or forensic == "":
        if f:
            logger.error(
                _(
                    '(Faulty delta. Please consider retrying with the option "--forensic=do" ).'
                )
                + "\n"
            )
        return
    ...
    if all([(z is None) for z in f]):
        ...
        return
    if (
        forensic[:4] in ("mutt", "mail")
        or forensic[:7] == "icedove"
        or forensic[:10] == "thunderbird"
    ):
        ...
        if ":" in forensic:
            ...
            forensic = forensic[:a]
        logger.warning(
            _("There were faulty deltas.")
            + " "
            + _("Now invoking the mail sender to send the logs.")
        )
            ...
            for z in f:
                if z:
                    for j in z:
                        tar.add(j, arcname=os.path.basename(j))
            tar.close()
            args = "to=%s,subject=delta_failures,attachment='file:///%s'" % (
                email,
                temptar,
            )
        ...
        logger.warning(
            _("There were faulty deltas.") + " " + _("Sending logs to server.")
        )
        temptar = tempfile.mktemp(suffix=".tgz")
        tar = tarfile.open(name=temptar, mode="w:gz")
        for z in f:
        ...
        datagen, headers = poster.encode.multipart_encode(
            {
                "auth_userid": "debdelta",
                "auth_password": "slartibartfast",
                "thefile": open(temptar, "rb"),
            }
        )
        # Create the Request object
        request = urllib2.Request(
            "http://debdelta.debian.net:7890/receive", datagen, headers
        )
        ...
        logger.warning(
            _("Faulty delta. Please send by email to %s the following files:\n") % EMAIL
        )
        for z in f:
            if z:
        ...
        return
    logger.warning(
        _('(Faulty delta. Please consider retrying with the option "--forensic=do" ).')
        + "\n"
    )
...
def elf_info(f):
    ...
    if len(elfheader) == 32:
        ...
        return (
            is_elf,
            EI_CLASS.get(ei_class),
            EI_DATA.get(ei_data),
            EI_OSABI.get(ei_osabi),
            ET_TYPE.get(e_type),
        )
    # , e_machine, e_version
    else:
...
def parse_prelink_conf():
    ...
    for a in open("/etc/prelink.conf"):
        ...
        if len(b) != 2:
            logger.warning(
                '  (sorry this line of /etc/prelink.conf cannot be parsed currently: "'
                + a
                + '")'
            )
            continue
...
def do_patch_(
    ...
    temp_name, temp_err_name, ret = system(
        ("ar", "xvo", delta), TD + "/PATCH", return_output=True, ignore_output=True
    )
    if ret:
    ...
    if DEBUG > 1:
        ...
        if os.path.isdir(olddeb):
            ...
            if dpkg_params["OLD/Status"] != "install ok installed":
                die(
                    "Error: package %s is not installed, status is %s."
        ...
        for orig, divert in s:
            ...
            if os.path.isfile(divert) and not os.path.islink(divert):
                ...
                if unprelink:
                    ...
                    if VERBOSE > 3:
                        logger.debug(
                            "    copying/unprelinking " + divert + " to " + tmpcopy
                        )
                    ...
                    if proc.returncode:
                        ...
                        if out[-39:] == "does not have .gnu.prelink_undo section":
                            ...
                            logger.warning(
                                "!!Prelink -u failed, it needs at least 50000KB of free disk space\n"
                            )
                            prelink_u_failed.append(de_bar(orig))
                            unprelink = False
                        else:
                            logger.warning(
                                "!!Prelink -u failed on %r : %r\n", tmpcopy, out
                            )
                            prelink_u_failed.append(de_bar(orig))
                            unprelink = False
                    prelink_time += time.time()
    ...
    for a in params:
        if "needs-old" == a:
            ...
            if not os.path.exists("/usr/bin/xdelta3"):
                raise DebDeltaError(
                    'This patch needs xdelta3. Please install the Debian package "xdelta3".',
                    retriable=True,
                )
            ...
            if not os.path.exists("/usr/bin/xdelta"):
                raise DebDeltaError(
                    'This patch needs xdelta. Please install the Debian package "xdelta".',
                    retriable=True,
                )
        elif "needs-bsdiff" == a:
            if not os.path.exists("/usr/bin/bsdiff"):
                raise DebDeltaError(
                    'This patch needs bsdiff. Please install the Debian package "bsdiff".',
                    retriable=True,
                )
        elif "needs-lzma" == a:
            if not os.path.exists("/usr/bin/lzma"):
            ...
            raise DebDeltaError(
                'patch says "' + a + "' and this is unsupported. Get a newer debdelta.",
                retriable=True,
            )
        elif params[a]:
            logger.warning(
                "WARNING patch says %r and this is unsupported. Get a newer debdelta.",
                a,
            )
    ...
    if localepurged and not DEBUG:
        ...
        raise DebDeltaError(
            "Error, " + str(len(localepurged)) + " locale files are absent."
        )
    ...
    if (
        this_deb_format == "preunpacked"
    ):  # do progress reporting and unpacking in filesystem
        def do_extension():
            return (
                "_"
                + str(random.randint(1, 9999)).rjust(4, "0")
                + "_debdelta_preunpacked"
            )
                ...
                for tarmember in dt:
                    if tarmember.isreg():
                        ...
                        while os.path.exists(
                            n
                        ):  # wont overwrite existing stuff, never ever
                            n = "/" + tarmember.name + do_extension()
                    else:
                        n = ""
                    a = (
        ...
        try:
            ...
            tar_thread = threading.Thread(
                target=do_tar, args=(os.fdopen(piper), preunpacked_filelist, tar_status)
            )
    ...
    script_time += (
        time.time()
    )  # for --format='preunpacked' this time also include data MD5
    ...
    if not FORENSIC:
        ...
        def fore():
            ...
            tempos(f)
            try:
                (temp_fd, temp_fore_name) = tempfile.mkstemp(
                    prefix="debforensic_" + params["NEW/Package"] + "_"
                )
    ...
    if this_deb_format == "deb":
        ...
        if DO_MD5:
            if "NEW/MD5sum" in params:
                if VERBOSE > 1:
                    logger.debug(
                        "  verifying MD5  for %r", os.path.basename(newdeb or delta)
                    )
                ...
                logger.warning(
                    " Warning! no MD5 was verified for %r",
                    os.path.basename(newdeb or delta),
                )
    elif this_deb_format == "unzipped":
        if DO_MD5:
            c, b, z = untar_control_in_deb(list_ar(TD + "NEW.file"))
            p = PopenPipe(
                ["ar", "p", (TD + "NEW.file"), c, "|"] + z, stdout=subprocess.PIPE
            )
            ...
            p = subprocess.Popen(
                ["ar", "p", (TD + "NEW.file"), "data.tar"], stdout=subprocess.PIPE
            )
            m = compute_md5(p.stdout)
            p.wait()
        ...
        m = compute_md5(
            subprocess.Popen(
                'ar p "%s" control.tar.gz | zcat' % (TD + "NEW.file"),
                stdout=subprocess.PIPE,
                shell=True,
            ).stdout
        )
...
def do_delta_(olddeb, newdeb, delta, TD, forensic_file=None, info=[]):
    ...
    class Script:
        ...
        def recompressing(self, new_filename, new_filename_ext, new_file_zip_head):
            ...
            self.fd.write(")\n")
        def recompressing_by_arg(
            self, new_filename, new_filename_ext, new_file_zip_head, new_file_size
        ):
            ...
            ar_line_unzipped = (
                b"data.tar".ljust(16)
                + self.ar_line[16:48]
                + str2bytes(str(new_file_size)).ljust(10)
                + b"`\n"
            )
            append_NEW_file(
                ar_line_unzipped
            )  # there is no extra char, tar is 512b blocks
            self.fd.write(
                'cat >> NEW.file \n elif test "$1" = "piped" \n then cat >> data.pipe \n fi )\n'
            )
    ...
    for o, l in ("OLD", ar_list_old), ("NEW", ar_list_new):
        ...
        p.close()
        if VERBOSE:
            logger.debug(" " + o + ": " + str.join(" ", [o[4:] for o in s]))
        info = info + s
    ...
    debdelta_conf.read(
        ["/etc/debdelta/debdelta.conf", expanduser("~/.debdelta/debdelta.conf")]
    )
    ...
    delta_uses_infifo = ("xdelta3-fifo" not in DISABLED_FEATURES) and (
        USE_DELTA_ALGO == "xdelta3"
    )
    ...
    if "NEW/Installed-Size" in params and "OLD/Installed-Size" in params:
        ...
        if free and free < (instsize * 1024 + +(2 ** 23) + MAXMEMORY / 6):
            raise DebDeltaError(
                " Not enough disk space (%dkB) for creating delta (needs %dkB)."
    ...
    def patch_append(f):
        ...
        if VERBOSE > 2:
            ...
            logger.debug(
                "   appending %r of size %d to delta, %3.2f%% of new .deb",
                f,
                a,
                (a * 100.0 / newdebsize),
            )
    ...
    def delta_files__(o, n, p, algo, outpiped, infifo):
        ...
        # http://deltup.sf.net
            ...
            script.write(
                "~/debdelta/bdelta-0.1.0/bpatch " + o + " " + n + " " + p + "\n"
            )
        # diffball
        # http://developer.berlios.de/projects/diffball/
            ...
            script.write(
                "~/debdelta/diffball-0.7.2/patcher " + o + " " + p + " " + n + "\n"
            )
        # rdiff
        elif algo == "rdiff":
        ...
        # xdelta does not deal with different endianness!
            ...
            script.write(
                "bunzip2 " + p + ".bz2 ; xdelta patch " + p + " " + o + " " + n + "\n"
            )
            p += ".bz2"
        elif algo == "xdelta":
            system(
            ...
            script.write(
                "~/debdelta/jdiff06/src/jpatch " + o + " " + p + " " + n + "\n"
            )
        else:
    ...
    def delta_files(o, n, outpiped=None, infifo=None):
        ...
        if VERBOSE > 1:
            logger.debug(
                "  compute delta for %s (%dkB) and %s (%dkB)",
                o,
                osize / 1024,
                n,
                nsize / 1024,
        ...
        if USE_DELTA_ALGO == "bsdiff" and osize > (1.1 * (MAXMEMORY / 12)) and VERBOSE:
            logger.debug(
                " Warning, memory usage by bsdiff on the order of %dMb",
                (12 * osize / 2 ** 20),
            )
        #
        p, this_delta_outpiped = delta_files__(
            o, n, p, USE_DELTA_ALGO, outpiped, infifo
        )
        # script.write(s)
        #
    ...
    def parse_gzip_header(n):
        ...
        if format == 0:
            ...
            logger.debug(
                " Warning: unsupported compression .gz format: %r %r", oct(format), n
            )
            return
        if myord(a[9]) != 3:
            if VERBOSE:
                logger.debug(
                    " Warning: unknown OS in .gz format: %r %r", oct(myord(a[9])), n
                )
    ...
    def delta_gzipped_files(o, n):
        ...
        pro = subprocess.Popen(
            ("zcat", TD + n), stdout=open(TD + p + ".new", "wb"), cwd=TD
        )
        pro.wait()
        if pro.returncode:
        ...
        for i in pack_list:
            ...
            gzip_flags = "-" + str(i)
            pro = subprocess.Popen(
                ("gzip", "-c", "-n", gzip_flags, TD + p + ".new"),
                stdout=open(TD + p + ".faked.gz", "wb"),
            )
            pro.wait()
            if pro.returncode:
            ...
            if i == pack_level and VERBOSE > 3:
                logger.debug(
                    "    warning: wrong guess to re-gzip to equal file: %r %r %r",
                    gzip_flags,
                    r,
                    n,
                )
        ...
        pro = subprocess.Popen(
            ("zcat", TD + o), stdout=open(TD + p + ".old", "wb"), cwd=TD
        )
        pro.wait()
        if pro.returncode:
        ...
        if check:
            ...
            while redo and PARS:
                ...
                if c.returncode:
                    ...
                    if VERBOSE or DEBUG:
                        logger.debug(
                            " Tried XZ options but failed: %r %r %r", par, crc, thread
                        )
    ...
    def delta_tar(
        ...
        assert type(old_filename) in string_types or isinstance(
            old_filename, FunctionType
        )

        script.write(
            'ECR () { $E "$1" ; $E "${FTH}" ; cat OLD/'
            + CWD
            + '/"$1" ; rm OLD/'
            + CWD
            + '/"$1" ;}\n'
        ...
        logger.log(
            logging.DEBUG if newtar.format == tarfile.GNU_FORMAT else logging.WARNING,
        ...
        for newtarinfo in newtar:
            ...
            if VERBOSE and (
                (t == "2" and a != 0o777)
                or (t == "0" and ((a & 0o400) == 0))
                or (t == "5" and ((a & 0o500) == 0))
            ):
                logger.debug(
                    " weird permission %r %r %r", newname, oct(a), repr(newtarinfo.type)
                )
            ###
            ...
            if VERBOSE > 3 and newname != de_bar(newname):
                logger.debug(
                    "    filename in new tar has weird ./ in front: %r", newname
                )
        ...
        for newname in newnames:
                ...
                if md5 in reverse_old_md5:
                    ...
                    if VERBOSE > 2:
                        if oldname == newname:
                            ...
                            logger.debug(
                                "   use identical old file: %r %r", oldname, newname
                            )
            ...
            if oldname is None and ne in oldnames_premangle:
                ...
                for o in oldnames_premangle[ne]:
                    ...
                    if VERBOSE > 3:
                        logger.debug(
                            "    name/len diff %.2f+%.2f=%.2f %r", slen, sfile, s, o
                        )
                    if s < basescore:
                        oldname = o
                        basescore = s
                if oldname and VERBOSE > 2:
                    logger.debug(
                        "   best similar %.3f %r %r", basescore, newname, oldname
                    )
            if not oldname:
                if VERBOSE > 2:
        ...
        def _append_(p, w, rm):
            ...
            while a:
                try:
                    ...
                    s = sys.exc_info()[1]
                    raise DebDeltaError(
                        " OSError (at _a_) while writing: " + str(s), True
                    )
                a = f.read(1024)
            f.close()
            if rm:
        ...
        def mega_cat_chunk(oldoffset, newoffset, background=True):
            ...
            while l < newoffset:
                ...
                try:
                    ...
                    s = sys.exc_info()[1]
                    raise DebDeltaError(
                        " OSError (at MCK) while writing: " + str(s), True
                    )
            f.close()
            of.close()
        ...
        for newtarinfo in newtar:
            ...
            if newtarinfo.isdir():
                ...
                script.write(
                    "$E '"
                    + prepare_for_echo(str2bytes(newtarinfo.name))
                    + '\'"${FTH}"\n'
                )
                if VERBOSE > 2:
            ...
            if (
                ...
                and not (
                    ...
                    and old_md5
                    and name in old_md5
                    and new_md5[name] == old_md5[name]
                )
            ):
                newtar.extract(newtarinfo, TD + "NEW/" + CWD)
                delta_gzipped_files(
                    "OLD/" + CWD + "/" + name, "NEW/" + CWD + "/" + name
                )

            if VERBOSE > 2:
                logger.debug(
                    "   adding reg file: %r %r", oldname, mul and "(multiple)" or ""
                )
        ...
        if os.path.getsize(TD + "/" + current_chunk_name) > 0:
            mega_cat_chunk(
                progressive_new_offset,
                os.path.getsize(TD + new_filename),
                background=False,
            )
        else:
        ...
        if new_filename == "NEW/data.tar":
            script.recompressing_by_arg(
                new_filename, new_filename_ext, new_file_zip_head, new_file_size
            )
        else:
    ...
    def decompress_pipe_from_ar(oldnew, name, p=None):
        ...
        if p == b".tar" or p == False:
            def x():
                return PopenPipe(
                    ["ar", "p", oldnew + ".file", name], stdout=subprocess.PIPE, cwd=TD
                )

        else:
            def x():
                return PopenPipe(
                    ["ar", "p", oldnew + ".file", name, "|"] + db[p],
                    stdout=subprocess.PIPE,
                    cwd=TD,
                )
    ...
    for name in ar_list_new:
        ...
        if newsize < 128:  # file is too short to compute a delta,
            ...
            script.write(
                "cat " + newname + cn + " >> NEW.file ;  rm " + newname + cn + "\n"
            )
            unlink(TD + newname)
            # pad new deb
            if extrachar:
    ...
    if forensic_file:
        forensics_rfc(
            forensic_file, info, True, control_forensic, data_forensic, old_conffiles
        )
    ...
    if "lzma" not in DISABLED_FEATURES and os.path.exists("/usr/bin/lzma"):
        ...
        patch_files.append(
            (os.path.getsize(TD + "PATCH/patch.sh.lzma"), "lzma", "patch.sh.lzma")
        )
    if "xz" not in DISABLED_FEATURES and os.path.exists("/usr/bin/xz"):
        system(("xz", "-q", "-9", "-k", "PATCH/patch.sh"), TD)
        patch_files.append(
            (os.path.getsize(TD + "PATCH/patch.sh.xz"), "xz", "patch.sh.xz")
        )
    system(("bzip2", "-q", "--keep", "-9", "PATCH/patch.sh"), TD)
    patch_files.append(
        (os.path.getsize(TD + "PATCH/patch.sh.bz2"), "bzip2", "patch.sh.bz2")
    )
    system(("gzip", "-q", "-9", "-n", "PATCH/patch.sh"), TD)
    patch_files.append(
        (os.path.getsize(TD + "PATCH/patch.sh.gz"), "gzip", "patch.sh.gz")
    )
    ...
    if VERBOSE:
        ...
        logger.info(
            " "
            + _(
                "delta is %(perc)3.1f%% of deb; that is, %(save)dkB are saved, on a total of %(tot)dkB."
            ),
            {
                "perc": percent,
                "save": ((newdebsize - deltasize) / 1024),
                "tot": (newdebsize / 1024),
            },
        )
        logger.info(
...
def iterate_Packages(packages, use_debian_822=True):
    ...
    assert os.path.basename(packages) in (
        "Packages",
        "Packages.gz",
        "Packages.bz2",
        "Packages.xz",
    ...
    for l in F:
        ...
        if l[:9] == "Package: ":
            ...
            if of is None or pack is None or vers is None or arch is None:
                logger.warning(
                    "Warning, skipping incomplete record in index: %r %r %r %r",
                    of,
                    pack,
                    vers,
                    arch,
...
def scan_deb_dir(d, debname, label, lazy, info_by_pack_arch, info_by_file):
    assert (debname is None or type(debname) in string_types) and type(
        label
    ) in string_types
...
def scan_deb_bydict(a):
    ...
    p = PopenPipe(
        ["ar", "p", a["File"], c, "|", "tar", b, "-xp", "-f", "-", "-O", "./control"],
        stdout=subprocess.PIPE,
    )
    scan_control(p.stdout, params=a)
    p.close()
...
def delta_dirname(f, altdir):
    ...
    if altdir is not None:
        if altdir[-2:] == "//":
            if "../" in f:
                logger.warning(
                    "Warning: cannot combine .. and // ! Saving in %r", altdir
                )
                return altdir
...
def iter_deltas_one_pack_arch(
    pa, ar, info_pack, thedir, theforensicdir, VersionCompare, loglevel=logging.ERROR
):
    ...
    while newest >= 0:
        ...
        if new["Label"] != "CMDLINE":
            if VERBOSE > 3:
                logger.debug(
                    "    Newest version deb was not in cmdline, skip down one: %r", new
                )
        else:
            break
        newest -= 1
    ...
    if VERBOSE > 2:
        logger.debug(
            ...
            ar,
            [
                (o.get("Version"), o["Label"], o.get("Packages"), o.get("File"))
                for o in info_pack
            ],
        )
    ...
    while oldn > 0:
        ...
        if theforensicdir:
            if "Filename" in new:
                forensicdirname = delta_dirname(
                    os.path.dirname(new["Filename"]), theforensicdir
                )
            elif "File" in new:
                forensicdirname = delta_dirname(
                    os.path.dirname(new["File"]), theforensicdir
                )
            else:
                assert 0
            forensicbasename = (
                pa + "_" + version_mangle(old["Version"]) + "_" + ar + ".forensic"
            )
...
def do_delta_and_test(old_File, new_File, delta, forensicfile):
    ...
    if free and (free < (newdebsize / 2 + 2 ** 15)):
        if VERBOSE:
            logger.warning(" " + (_("Not enough disk space for storing `%s'.")), delta)
    ...
    if DO_TEST:
        ...
        try:
            ...
            if os.path.exists(deltatmp):
                if KEEP:
                    logger.warning(
                        " " + _("You may want to examine:") + " " + str(deltatmp)
                    )
                else:
                    os.unlink(deltatmp)
            if not s.retriable:
...
def init_apt_return_VersionCompare():
    try:
        ...
        raise DebDeltaError(
            'python module "apt_pkg" is missing. Please install python-apt',
            retriable=True,
        )
...
def do_deltas(debs):
    ...
    if not debs:
        ...
        logger.warning(
            _("Warning, no --old arguments, debdeltas will not generate any deltas.")
        )
        if not DEBUG:
            return
        ...
        if os.path.isfile(arg):
            if os.path.basename(arg) in ("Packages", "Packages.gz", "Packages.bz2"):
                for a in iterate_Packages(arg):
                    a[
                        "Label"
                    ] = "CMDLINE"  # note that, if a file is scanned more than once, the last label is CMDLINE
                    # DEBUG a['Packages'] = arg
                    ...
                    for alt in ALT_DOUBLEDIR:
                        scan_deb_dir(
                            delta_dirname(ofd, alt), pa, "ALT", True, info_by_pack_arch
                        )
                    if CLEAN_DELTAS:
                        scan_delta_dir(
                            delta_dirname(ofd, DIR),
                            pa,
                            scanned_delta_dirs,
                            old_deltas_by_pack_arch,
                        )
    ...
    for pa, ar in info_by_pack_arch:
        ...
        if CLEAN_DELTAS:
            if (pa, ar) in old_deltas_by_pack_arch:
                versions = [
                    (o["Version"]) for o in info_pack if (o["Label"] == "CMDLINE")
                ]
                    ...
                    if n_d not in versions:
                        if os.path.exists(f_d):
                            if os.stat(f_d)[ST_MTIME] < timegrace:
                                ...
                                logger.debug(
                                    "  Obsolete but still young, graced: %r", f_d
                                )
                        else:
...
class Predictor:
    ...
    def update(self, p, t):
        ...
        if VERBOSE > 1:
            logger.debug(
                "  Upstream %r PatchSpeedRatio from %r to %r , %r",
                ut,
                r,
                nr,
                self.upgrade_stats[s]["PatchSpeedRatio"],
            )
...
def delta_uri_from_config(config, **dictio):
    ...
    for s in secs:
        ...
        if "delta_uri" not in opt:
            raise DebDeltaError(
                "sources.conf section " + repr(s) + "does not contain delta_uri",
                exitcode=3,
            )
        match = True
        for a in dictio:
    ...
    if VERBOSE:
        logger.warning(
            " " + _("(sources.conf does not provide a server for `%s')"),
            repr(dictio["PackageName"]),
        )
...
def delta_upgrade_(args):
    ...
    try:
        ...
        if sys.version_info.major == 2:
            logger.info(
                "You may want to install `requests`\n# sudo apt install python-requests\n"
            )
        else:
            logger.warning(
    ...
    config = ConfigParser()
    a = config.read(
        ["/etc/debdelta/sources.conf", expanduser("~/.debdelta/sources.conf")]
    )
    ...
    try:
        ...
        raise DebDeltaError(
            'python module "apt_pkg" is missing. Please install python-apt', True
        )
    ...
    try:
        ...
        raise DebDeltaError(
            'python module "apt" is missing. Please install a newer version of python-apt (newer than 0.6.12).',
            True,
        )
    ...
    try:
        ...
        if DO_PROGRESS:
            sys.stderr.write(
                (_("Failed! Safe upgrading APT cache...")).ljust(terminalcolumns) + "\r"
            )
        elif VERBOSE:
        ...
        while True:
            ...
            if not ACT:
                ...
                try:
                    ...
                    msg = _(
                        "Created,    time %(time)5.2fsec, speed %(speed)4s/sec, %(name)s"
                    )
                    msgdata = {
                        "time": a,
                        "speed": SizeToKibiStr(l / (a + 0.001)),
                        "name": os.path.basename(newdeb),
                    }
                    if DO_PROGRESS:
    ...
    def _connect(uri, headers):
        ...
        if requests and uri_p.scheme in ("http", "https"):
            ...
            try:
                ...
                return (
                    req,
                    req.raw.status,
                    getattr(req.raw, "msg", "(no message)"),
                    req.raw.info(),
                )
            ...
            try:
                ...
                return (
                    req,
                    getattr(req, "code", None),
                    getattr(req, "msg", "(no message)"),
                    req.info(),
                )
        ...
        if uri_p.scheme == "http" and not proxies:
            ...
            try:
                ...
                e = sys.exc_info()[1]
                if VERBOSE:
                    puke(" Connection error (retrying): %r" % (uri_p[1],), e)
                conn_close(uri)
                try:
                    conn = conn_by_url(uri)
                    conn.request("GET", urlquote(uri_p[2]), headers=headers)
    ...
    def download_10k_uri(uri, outname):
        ...
        if status == 301 and "location" in responseheaders:
            ...
            outnametemp = os.path.join(
                os.path.dirname(outname), "partial", os.path.basename(outname)
            )
            try:
    ...
    def download_uri(uri, outname, conn_time, len_downloaded, statusdb):
        ...
        if status in (301, 302) and "location" in responseheaders:
            ...
            if VERBOSE > 1:
                logger.debug(
                    _("  Redirect to:") + " " + repr(responseheaders["location"])
                )
            return download_uri(
        ...
        if l and status == 200:
            logger.warning(
                " Hmmm... our HTTP range request failed, %r %r %r", re, status, message
            )
        if status == 200:
            out = open(outnametemp, "wb")
            try:
        ...
        if total_len is not None and free and (free + 2 ** 14) < total_len:
            logger.warning(
                _("Not enough disk space to download:") + " " + os.path.basename(uri)
            )
            r.read()
            r.close()
            mainexitcodes.append(1)
    ...
    for p in cache:
        ...
        if DO_PROGRESS:
            ...
            if 0 == (progress_count & 2047):
                sys.stderr.write(
                    ("%2.1f%%" % (float(progress_count) * 100.0 / len(cache))).ljust(
                        terminalcolumns
                    )
                    + "\r"
                )
        ...
        if is_installed(p) and marked_upgrade(p):
            ...
            newdebs = [
                p.shortname + "_" + candidate_version + "_" + arch + ".deb",
            ...
            if ":" in candidate_version:
                ...
                newdebs.append(
                    p.shortname + "_" + a[0] + "%3A" + a[1] + "_" + arch + ".deb"
                )
                newdebs.append(
                    p.shortname + "_" + a[0] + "%3a" + a[1] + "_" + arch + ".deb"
                )
            ...
            for newdeb in newdebs:
                if os.path.exists(DEB_DIR + "/" + newdeb) or os.path.exists(
                    "/var/cache/apt/archives/" + newdeb
                ):
                    if VERBOSE > 1:
                        logger.debug(
                            "  Already downloaded: %r %r", p.name, candidate_version
                        )
                    newdeb = None
                    break
            if newdeb is None:
            ...
            if VERBOSE > 1:
                logger.debug(
                    "  Looking for a delta for %s from %s to %s ",
                    p.name,
                    installed_version,
                    candidate_version,
                )
            ...
            # delta name
            delta_name = delta_base_name(
                p.shortname, installed_version, candidate_version, arch
            )
            ...
            if a:
                ...
                available_deltas.append(
                    (l, p.name, uri, abs_delta_name, newdeb, deb_uri, a, True)
                )
                continue
            ...
            if status != 200 and status != 206 and status != 404:
                logger.warning(
                    "Delta is not downloadable (%s):%s", http_responses_get(status), uri
                )
                continue
            ...
            if status == 404:
                ...
                if (
                    uri[:7] == "http://"
                    and not proxies
                    and 200 == test_uri(uri + "-queued")
                ):  # FIXME support ftp or proxies
                    logger.info(
                        _("Delta is not yet ready in the server:") + " " + delta_name
                    )
                    if "q" in DEB_POLICY:
                        no_delta.append((deb_uri, newdeb))
                    elif VERBOSE > 1:
                ...
                if (
                    uri[:7] == "http://"
                    and not proxies
                    and 200 == test_uri(uri + "-fails")
                ):  # FIXME support ftp or proxies
                    logger.info(
                        _("Delta missing, server failed to create it:")
                        + " "
                        + delta_name
                    )
                    if "f" in DEB_POLICY:
                ...
                if newsize <= MIN_DEB_SIZE:  # check only on small packages
                    logger.info(
                        _("Delta was not created since new package is too small:")
                        + " "
                        + delta_name
                    )
                    if "t" in DEB_POLICY:
    ...
    if DEBUG or VERBOSE:
        ...
        logger.info(
            ...
            % {
                ...
                "speed": SizeToKibiStr(
                    deltas_down_size / float(deltas_down_time + 0.001)
                ),
            }
        )
        if available_deltas:
            logger.info(
                " " + _("Need to get %s of deltas."),
                SizeToKibiStr(sum([a[0] for a in available_deltas])),
            )
    ...
    if DEB_FORMAT == "deb" and a < c:
        logger.warning(
            "**" + _("Very low disk space, need %(need)d kB have %(have)d kB"),
            {"need": c, "have": a},
        )
    ...
    if DO_PROGRESS and terminalcolumns > 4:
        progress_thread = threading.Thread(
            target=print_progress, args=(thread_returns,)
        )
    ...
    for (
        ...
        if not os.path.exists(abs_delta_name):
            ...
            r = download_uri(
                uri, abs_delta_name, deltas_down_time, deltas_down_size, thread_returns
            )
            del thread_returns["downloaduri"]
            if r is None or isinstance(r, HTTPException):
                if VERBOSE:
                    logger.info(
                        " " + _("You may wish to rerun, to get also:") + " " + uri
                    )
                continue
            else:
    ...
    if patching_thread.is_alive() and no_delta and VERBOSE > 1:
        logger.info(
            "  Downloading deltas done, downloading debs while waiting for patching thread."
        )
    while patching_thread.is_alive() or ("a" in DEB_POLICY and no_delta):
        if no_delta:
            ...
            thread_returns["downloaduri"] = os.path.basename(uri)
            r = download_uri(
                uri, newdeb, debs_down_time, debs_down_size, thread_returns
            )
            del thread_returns["downloaduri"]
            if r is None or isinstance(r, HTTPException):
                if VERBOSE:
    ...
    while progress_thread is not None and progress_thread.is_alive():
        time.sleep(0.1)
    ...
    if VERBOSE:
        if deltas_down_time:
            ...
            logger.info(
                ...
                + msg
                % {
                    "size": SizeToKibiStr(a),
                    "time": int(t),
                    "speed": SizeToKibiStr(a / t),
                }
            )
    ...
    if total_time:
        ...
        logger.info(
            " "
            + _(
                "total resulting debs, size %(size)s time %(time)dsec virtual speed %(speed)s/sec"
            )
...
if action == "patch":
    ...
    for v in argv:
        if not (os.path.isfile(v) or os.path.isdir(v)):
            sys.stderr.write(
                _("Error: argument is not a directory or a regular file:") + " " + v
            )
            raise SystemExit(3)
    try:
    ...
    try:
        ...
        logger.error(
            'ERROR!!! python module "apt_pkg" is missing. Please install python-apt'
        )
        raise SystemExit(1)
    ...
    for a in argv:
        ...
        if not candidate.uris:
            logger.warning(
                _("Sorry, cannot find an URI to download the debian package of `%s'.")
                % a
            )
            continue
        deb_uri = candidate.uri
        ...
        if installed_version == candidate_version:
            logger.warning(
                _("Sorry, the package `%s' is already at its newest version.") % a
            )
            continue
