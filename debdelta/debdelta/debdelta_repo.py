...
class theSQLdb:
    ...
    def __init__(self, dbname):
        ...
        self.dbname = dbname
        self.sql_connection = dbapi.connect(
            dbname, detect_types=dbapi.PARSE_DECLTYPES | dbapi.PARSE_COLNAMES
        )
    ...
    def add_one(
        ...
        self.sql_cursor.execute(
            "SELECT name,version,arch,id FROM package WHERE filename = ? ", (filename,)
        )
        tp = self.sql_cursor.fetchone()
        if tp:
            if tp[0] != name or tp[1] != version or tp[2] != arch:
                sys.stderr.write(
                    "Filename already in package database as: %s\n" % repr(tp)
                )
                return
            tpid = tp[3]
        else:
        ...
        self.sql_cursor.execute(
            "SELECT id FROM dist WHERE package_id = ? AND disttoken = ? ",
            (tpid, disttoken),
        )
        td = self.sql_cursor.fetchone()
        if td:
            sys.stderr.write(
                "Package,version,arch already in dist database for this disttoken\n"
            )
            # FIXME we may have added a package and no dist?
            return
        self.sql_cursor.execute(
            "INSERT INTO dist VALUES (null, ?, ?, ?, ?)",
    ...
    def package_versions(self, name, disttoken, arch=None, generated=None):
        ...
        if generated is None:
            sql_cursor1.execute(
                "SELECT package_id FROM dist WHERE disttoken = ? ", (disttoken,)
            )
        elif generated:
            sql_cursor1.execute(
                "SELECT package_id FROM dist WHERE disttoken = ? AND generate = 1",
                (disttoken,),
            )
        else:
            sql_cursor1.execute(
                "SELECT package_id FROM dist WHERE disttoken = ? AND generate = 0",
                (disttoken,),
            )
        z = []
        for a in sql_cursor1:
            if arch:
                sql_cursor2.execute(
                    "SELECT id,name,arch,version FROM package WHERE id = ? AND arch = ?",
                    (a[0], arch),
                )
            else:
                sql_cursor2.execute(
                    "SELECT id,name,arch,version FROM package WHERE id = ?", (a[0])
                )
    ...
    def create_deltas(self):
        ...
        sql_cursor2 = self.sql_connection.cursor()
        sql_cursor1.execute(
            "SELECT package_id,disttoken FROM dist WHERE generated = 0 "
        )
...
def create(dbname):
    if os.path.exists(dbname):
        sys.stderr.write(
            sys.argv[0] + ": will not overwrite already existing " + dbname + "\n"
        )
        sys.exit(1)
    ...
    if stdin:
        for a in sys.stdin:
            ...
            if len(b) == 5:
                ...
                sys.stderr.write(
                    "It is not a tab separated list of 5 elements: %s\n" % repr(a)
                )
    else:
        if len(argv) == 5:
            H.add_one(*argv)
        else:
            sys.stderr.write("It was not given 5 arguments: %s\n" % repr(argv))
    H.commit()
...
def sos(dbname, workspace, argv):
    ...
    if len(argv) != 1:
        ...
        sys.exit(1)
    H.sql_cursor.execute(
        "SELECT id,name,version,arch FROM package WHERE filename = ? ", argv
    )
