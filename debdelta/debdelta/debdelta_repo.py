...
class theSQLdb:
    ...
    def __init__(self, dbname):
        ...
        self.dbname = dbname
        self.sql_connection = dbapi.connect(dbname, detect_types=dbapi.PARSE_DECLTYPES | dbapi.PARSE_COLNAMES)
    ...
    def add_one(self, name, version, arch, filename, disttoken, generated=0, ownfile=0, ctime=None):
        if ctime is None:
            ctime = int(time.time())
        self.sql_cursor.execute("SELECT name,version,arch,id FROM package WHERE filename = ? ", (filename,))
        ...
        if tp:
            ...
            self.sql_cursor.execute(
                "INSERT INTO package VALUES (null, ?, ?, ?, ?, ?, ?)", (name, version, arch, filename, ownfile, ctime)
            )
        ...
        if td:
            ...
            return
        self.sql_cursor.execute("INSERT INTO dist VALUES (null, ?, ?, ?, ?)", (disttoken, tpid, generated, ctime))
    ...
    def package_versions(self, name, disttoken, arch=None, generated=None):
        ...
        for a in sql_cursor1:
            if arch:
                sql_cursor2.execute("SELECT id,name,arch,version FROM package WHERE id = ? AND arch = ?", (a[0], arch))
            else:
    ...
    if stdin:
        for a in sys.stdin:
            ...
            if len(b) == 5:
                ...
                sys.stderr.write("It is not a tab separated list of 5 elements: %s\n" % repr(a))
    else:
        if len(argv) == 5:
            H.add_one(*argv)
        else:
...
def main():
    ...
    try:
        (opts, argv) = getopt.getopt(
            sys.argv[1:], "hvdW:", ("help", "debug", "verbose", "workspace=", "add", "stdin", "sos", "create", "deltas")
        )
