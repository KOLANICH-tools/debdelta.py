...
Possible commands:
 ...
 test_gpg_signing                  test if it can gpg-sign a file
...
# Copyright (c) 2013-2020 A. Mennucci
# License: GNU GPL v2
...
logging.basicConfig(
    format="%(asctime)s %(levelname)-7s %(funcName)s %(message)s", level=logging.INFO
)
...
if DO_DAEMON:
    ...
    daemon_context.open()
    # FIXME these are logged nowhere
    logger.info("Running as PID %d", os.getpid())
    logger.info("Running on Python: %r", sys.version)
...
def send_temp_out_as_mail(out):
    ...
    if os.path.getsize(out.name) > 0:
        ...
        if email:
            p = subprocess.Popen(
                ["mail", "-s", repr(args), email], stdin=open(out.name), **Popen_as_text
            )
            p.wait()
            if p.returncode:
                if logger:
...
if DO_MAIL:
    if temp_out is None:
        temp_out = tempfile.NamedTemporaryFile(
            prefix="debdeltas_server_out", delete=False
        )
...
logger_hdlr = logging.handlers.RotatingFileHandler(
    join(logs_dir, "debdeltas_server"), maxBytes=2 ** 24, backupCount=30
)
logger_formatter = logging.Formatter(
    "%(asctime)s %(process)d %(levelname)-7s %(funcName)s %(message)s"
)
...
def my_subprocess_Popen(cmd, **kwargs):
    ...
    if "stdout" not in kwargs:
        kwargs["stdout"] = LoggerWriter(
            prefix=("%r stdout=" % cmd[0]), level=logging.INFO
        )
    if "stderr" not in kwargs:
        kwargs["stderr"] = LoggerWriter(
            prefix=("%r stderr=" % cmd[0]), level=logging.ERROR
        )
...
def mirrors_binary_Packages(mirrorsdir=mirrorsdir, archs=architectures):
    ...
    for m in distrib:
        for d in distrib[m]:
            ...
            for k in components:
                for a in archs:
                    z = LiPa_find_extension(
                        mirrorsdir, [m, "dists", d, k, "binary-" + a, "Packages"]
                    )
                    if z:
...
class Release_as_dict(object):
    def __init__(self, a, thehash):
        ...
        if debian_deb822 is None:
            logger.error(
                "this function needs module debian.deb822 from package `python3-debian`"
            )
        elif a and os.path.isfile(a):
            self.rele = {
                z["name"]: z[self.thehash]
            ...
            if a:
                logger.warning("file not found:" + repr(a))
            self.rele = {}
            self.filename = None
...
def _backup_Packages(now, where):
    ...
    OldRelease = {}
    if debian_deb822 is None:
        logger.warning(" please install package 'python3-debian' ")
    for m in distrib:
        for d in distrib[m]:
            # read and copy Release file
            if oldwhere:
                OldRelease[(m, d)] = Release_as_dict(
                    join(oldwhere, m, "dists", d, "Release"), thehash
                )
            else:
    ...
    for p in iterate_Packages_in_dict_of_mirrors(distrib):
        ...
        if new:
            ...
            logger.warn(
                " failed to get %r for %r from new Release %r"
                % (thehash, B, NewRelease[m_d].filename)
            )
        if oldwhere and new:
            ...
            if os.path.isfile(o_p):
                ...
                if not old:
                    if OldRelease[m_d]:
                        logger.warn(
                            " failed to get %r for %r from old Release %r"
                            % (thehash, B, OldRelease[m_d].filename)
                        )
                elif old == new:
                    linked_p = True
        ...
        reversed(
            sorted(
                [
                    ...
                    if (
                        a
                        and os.path.isdir(join(backup_Packages_dir, a))
                        and a[0] == "2"
                    )
                ]
            )
...
class Compute_Priority(object):
    ...
    def __init__(self):
        ...
        if os.path.isfile(popcon_cache):
            self.popcon_dict = pickle.load(open(popcon_cache, "rb"))
        else:
    ...
    def __call__(self, old, new):
        ...
        return int(
            priority_for_mirrors.get(m, 0.0)
            + priority_by_popcon * (1.0 - self.popcon_dict.get(old["Package"], 0.0))
        )
...
def _scan_backups_queue_deltas(bPd=backup_Packages_dir):
    ...
    for old, new, delta, forensicfile in iter_backups_for_deltas(bPd):
        ...
        if x:
            logger.debug(
                "already queued %r as %r ", (priority, old, new, delta, forensicfile), x
            )
        elif (
            os.path.isfile(delta)
            or os.path.exists(delta + "-too-big")
            or os.path.exists(delta + "-fails")
        ):
            # fixme should check also if the new deb is too small
            logger.debug(
                "exists, not queued: %r", (priority, old, new, delta, forensicfile)
            )
        else:
            # this may actually requeue already queued records...
            # it is ok since they will have newer timestamps
            thesqldb.queue_add(
                priority, old["File"], new["File"], shortdelta, forensicfile
            )
...
def set_environ_gpg_agent():
    ...
    elif gnupg_agent_mode == "load":
        if os.path.isfile(gnupg_agent_info_file):
            gpg_agent_info = (
                open(gnupg_agent_info_file).readline().strip().split("=")[-1]
            )
            if gpg_agent_info:
                os.environ["GPG_AGENT_INFO"] = gpg_agent_info
        else:
            # environment is unused for gnupg >= 2.1
            logger.warn(" file not found: %r" % gnupg_agent_info_file)
    elif gnupg_agent_mode == False:
        if "GPG_AGENT_INFO" in os.environ:
            del os.environ["GPG_AGENT_INFO"]
    else:
        logger.warn(
            "configuration variable 'gpg_agent_info' set to a strange value: "
            + repr(gnupg_agent_info)
        )
...
def worker_creates_deltas(exit_when_queue_empty=False):
    r = start_gpg_agent()
    # if r != 0: return ## currently the agent is not really needed
    r = test_gpg_signing()
    if r != 0:
        return
    with lockfile.FileLock(worker_creates_deltas_lockname, timeout=lock_timeout) as L:
        ...
        open(L.path, "wb").write(
            pickle.dumps(
                {"exit_when_queue_empty": exit_when_queue_empty, "pid": os.getpid()}
            )
        )
    ...
    while True:
        ...
        if True:
            if x is None:  # queue is empty, or error
                ...
                delta = x[E.delta]
                shortdelta = (
                    delta[len(deltas_www_dir) + 1 :] if delta[0] == "/" else delta
                )
                ...
                if ret:
                    if 0 == ret[0]:
                        ...
                        if os.path.exists(
                            y[E.delta]
                        ):  # do not append if the delta was too big
...
def move_from_transit_to_server():
    p = subprocess.Popen(
        ["find", "-type", "f", "-not", "-name", "*_tmp_"],
        cwd=deltas_transit_dir,
        stdout=subprocess.PIPE,
        **Popen_as_text
    )
    moved = []
    ...
    for j in p.stdout:
        ...
        if os.path.isfile(a):
            ...
            ndequeud += 1
    logger.info(
        "moved %d files from transit, %d queue timestamps removed", len(moved), ndequeud
    )
    p.wait()
    return moved
    ...
    if x and ret and thesqlhistory:
        ...
        try:
            ...
            thesqlhistory.add(
                ...
                delta_elaps,
                patch_elaps,
                x[E.forensic][len(forensic_www_dir) + 1 :]
                if x[E.forensic] and os.path.exists(x[E.delta])
                else None,
                error_string,
                None,
                1,
...
class Publisher(object):
    ...
    def start(self, created=[]):
        ...
        if os.path.isfile(publisher):
            ...
            p = my_subprocess_Popen(args + created, **Popen_as_text)
            self.proc = p
    ...
    def _log_publishing(self):
        if self.proc and self.returncode is not None:
            if self.returncode:
                logger.error(
                    "failed: exitstatus %r args %r deltas %r",
                    self.returncode,
                    self.args,
                    self.created,
                )
...
def update_popcon():
    ...
    popcon_dict = {}
    maximum = None
    for a in my_subprocess_Popen(
        popcon_update_command, shell=True, stdout=subprocess.PIPE, **Popen_as_text
    ).stdout:
        if not a or a[0] == "#":
            continue
    ...
    if popcon_dict:
        ...
        pickle.dump(popcon_dict, open(popcon_cache + "~~", "wb"))
...
def daemonize_maybe_mail(cmdname, cmd, *args, **kw):
    ...
    try:
        ...
        if os.path.getsize(out.name) > 0:
            logger.debug("there was output, sending email")
            p = my_subprocess_Popen(
                ["mail", "-s", cmdname, email], stdin=subprocess.PIPE, **Popen_as_text
            )
            o = open(out.name)
            for a in o:
                p.stdin.write(a)
...
def start_gpg_agent():
    ...
    try:
        version = subprocess.check_output(
            ["gpg-agent", "--homedir", gnupg_home, "--version"], **Popen_as_text
        )
        agenthelp = subprocess.check_output(
            ["gpg-agent", "--homedir", gnupg_home, "--help"], **Popen_as_text
        )
    except subprocess.CalledProcessError:
        logger.error("Cannot run gpg-agent --version or --help")
        return False
    try:
        version = version.split("\n")[0].split()[-1]
        version = list(map(int, version.split(".")))
    except BaseException:
        logger.error("cannot parse gpg-agent version " + repr(version))
    has_env = "--write-env-file" in agenthelp
    logger.info("gpg-agent version %r , supports environ %r" % (version, has_env))
    #
    if has_env:
        r = start_gpg_agent_environ()
    else:
        r = start_gpg_agent_noenviron()
    if r == 0:
        r = test_gpg_signing()
    return r

def start_gpg_agent_noenviron():
    p = subprocess.Popen(
        ["gpg-connect-agent", "--homedir", gnupg_home, "--no-autostart"],
    ...
    magik = "3PlOBJK"
    try:
        p.stdin.write("/echo " + magik)
    except IOError:
        pass  # maybe the process has already exited.. broken pipe
    p.stdin.close()
    s = p.stdout.read()
    p.wait()
    if p.returncode == 0 and s[: len(magik)] == magik:
        logger.info("agent is already running")
        r = 0
    else:
        logger.info("starting agent")
        r = subprocess.call(["gpg-agent", "--homedir", gnupg_home, "--daemon"])
    return r

def start_gpg_agent_environ():
    # ignore environment ?
...
def test_gpg_signing():
    ...
    n.write(b"pippo\n")
    n.close()
    e = tempfile.NamedTemporaryFile(delete=False)
    p = subprocess.Popen(
        [
            "gpg",
            "--quiet",
    ...
    e.close()
    if r:
        logger.error("test signing fails, return code " + repr(r))
        logger.error(repr(open(e.name).read()))
    os.unlink(n.name)
    os.unlink(e.name)
    return r
...
if not args:
    ...
    if L.is_locked():
        try:
            D = pickle.load(open(L.path, "rb"))
        except BaseException:
            logger.exception(" while reading lock info %r", L.path)
        if not D:
            logger.warning("File %r is corrupt; breaking lock.", L.path)
            L.break_lock()
        if D.get("exit_when_queue_empty", True):
            # after this time, either it has exited, or it has noted that other deltas have been queued
            time.sleep(120)
        else:
            logger.debug(
                " backup,queue,worker : a persistent worker is present, exiting"
            )
    if not L.is_locked():
...
elif args[0] == "test_gpg_signing":
    return_code = test_gpg_signing()
