...
class Release_as_dict(object):
    def __init__(self, a, thehash):
        ...
        if debian_deb822 is None:
            logger.error('this function needs module debian.deb822 from package `python3-debian`')
            ...
            if a: logger.warning('file not found:'+repr(a))
            self.rele={}
            self.filename=None
    ...
    for m in distrib:
            for d in distrib[m]:
                ...
                if os.path.isfile(b+'.gpg'):
                    ...
                    p=my_subprocess_Popen(['gpgv','-q', b+'.gpg',b ], stdout = subprocess.PIPE,
                                          stderr=subprocess.STDOUT, **Popen_as_text)
                    e=p.stdout.read()
                    p.wait()
                    if p.returncode:
...
def worker_creates_deltas(exit_when_queue_empty=False):
    ...
    with lockfile.FileLock(worker_creates_deltas_lockname, timeout=lock_timeout) as L:
        ...
        open(L.path,'wb').write(pickle.dumps({'exit_when_queue_empty':exit_when_queue_empty,'pid':os.getpid()}))
        _worker_creates_deltas(exit_when_queue_empty=exit_when_queue_empty)
        logger.debug('end')
        if os.path.exists(L.path): os.unlink(L.path)
    ...
    while True:
        ...
        if True:
            ...
            if publish.poll() is not None and \
               ( (created and time.time() > publisher_interval + last_publishing) or \
...
def move_from_transit_to_server():
    p=subprocess.Popen(['find','-type','f', '-not', '-name', '*_tmp_'],
                       cwd=deltas_transit_dir,stdout=subprocess.PIPE, **Popen_as_text)
    moved=[]
    ndequeud=0
    for j in p.stdout:
        j=j[:-1]
        ...
        if os.path.isfile(a):
            ...
            os.unlink(a)
            ndequeud+=1
    logger.info('moved %d files from transit, %d queue timestamps removed',\
                len(moved), ndequeud)
    p.wait()
    return moved
...
def update_popcon():
    ...
    popcon_dict={}
    maximum=None
    for a in my_subprocess_Popen(popcon_update_command, shell=True,
                                 stdout=subprocess.PIPE, **Popen_as_text).stdout:
        if not a or a[0] == '#':
            continue
...
def start_gpg_agent():
    ...
    try:
        version=subprocess.check_output(['gpg-agent','--homedir', gnupg_home,'--version'],
                                        **Popen_as_text)
        agenthelp=subprocess.check_output(['gpg-agent','--homedir', gnupg_home,'--help'],
                                          **Popen_as_text)
...
def start_gpg_agent_noenviron():
    p=subprocess.Popen(['gpg-connect-agent','--homedir', gnupg_home,'--no-autostart'],
...
def test_gpg_signing():
    ...
    p=subprocess.Popen(['gpg', '--quiet', '--batch', '--homedir', gnupg_home,\
                     '-o', '/dev/null', '-u' , gnupg_key , '--sign', n.name],
                       stdin=sys.stdin, stderr=e, stdout=e)
    p.wait()
    r=p.returncode
    e.close()
    if r:
