...
class SQL_history(object):
    ...
    def _connect(self):
        return dbapi.connect(
            self.dbname, isolation_level="DEFERRED"
    ...
    def add(
        ...
        with self.sql_connection:
            self.sql_connection.execute(
                "INSERT INTO deltas_history VALUES (null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    distribution,
                    package,
                    architecture,
    ...
    def iterate_since(self, since):
        ...
        cursor.execute(
            "SELECT * FROM deltas_history WHERE ctime > ? ORDER BY ctime DESC ",
            (since,),
        )
        return cursor
...
def html_one_day(db, W):
    ...
    for x in s.iterate_since(since):
        ...
        if x[FE.new_size] and x[FE.delta_size]:
            percent = "%.1f%%" % (100.0 * x[FE.delta_size] / x[FE.new_size])
        else:
            percent = "--"
