...
import sys , os , tempfile , string ,getopt , tarfile , shutil , time, traceback, subprocess, time, tarfile, stat, hashlib, random, gzip
...
def untar_control_in_deb(ar_ls):
  ...
  zdb={'.gz' : ['gzip','-cd'], '.bz2': ['bzip2','-cd'],
      '.lzma':['unlzma','-c'], '.xz': ['unxz','-c'] }  
...
def do_patch_(delta, olddeb, newdeb, TD, runtime,
  ...
  if this_deb_format == 'deb':
    ...
    if DO_MD5:
      ...
      else: logger.warn(' Warning! no MD5 was verified for %r',os.path.basename(newdeb or delta))
...
def do_delta_(olddeb, newdeb, delta, TD, forensic_file=None, info=[]):
  ...
  for o,l in ('OLD',ar_list_old) , ('NEW',ar_list_new):
      ...
      p.close()
      if  VERBOSE  : logger.debug(' '+o+': '+join([o[4:] for o in  s],' '))
      info = info + s
      del s,p
...
##     print '  The old debian package ',olddeb,' does not contain a file list?!?' 
  ...
  def parse_gzip_header(n):
    ...
    if format ==  0 :
      ...
      return
    if a[9] != '\003' :
      if VERBOSE : logger.debug(' Warning: unknown OS in .gz format: %r %r',oct(ord(a[9])),n)
    ...
    if check:
      ...
      while redo and cmd_list:
        ...
        w=PopenPipe(['zcat',z.name,'|']+wcmd+['|','cmp','-',z.name],\
                    stdout=open(os.devnull,'w'),stderr=open(os.devnull,'w'),close_fds=True)
        redo=False
        w.wait()
        if w.returncode :
          redo=True
  ...
  def guess_xz_parameters(o, check=True):
    ...
    for a in b.stdout:
      ...
      if a[0]=='block':
        if crc and crc != a[9]:
          logger.warn("  warning : this xz -- compressed file was compressed with variable blocks crc ?! '%s' != '%s'",crc,a[9])
        crc=a[9]
        if par and par != a[15]:
          logger.warn("  warning : this xz -- compressed file was compressed with variable blocks options ?! '%s' != '%s'",par,a[15])
        par=a[15]
...
def do_delta_and_test(old_File, new_File, delta, forensicfile):
        ...
        if free and (free < (newdebsize /2 + 2**15)) :
          if VERBOSE : logger.warn(' '+(_("Not enough disk space for storing `%s'.")), delta)
...
def delta_uri_from_config(config, **dictio):
  ...
  if VERBOSE : logger.warn(' '+_("(sources.conf does not provide a server for `%s')"), repr(dictio['PackageName']))
...
def delta_upgrade_(args):
  ...
  if VERBOSE and proxies:
    ...
    logger.warn(_(' Proxy settings detected in the environment; using "urllib2" for downloading; but\n  this disables some features and is in general slower and buggier. See man page.'))
  ...
  def download_uri(uri, outname, conn_time, len_downloaded, statusdb):
      ...
      if status == 200 :
        ...
        try:
          ...
          e=sys.exc_info()[1]
          logger.warn('! problem, http response [206], Content Range %s , error %s , uri %s\n' %\
  ...
  for p in cache :
    ...
    if is_installed(p) and marked_upgrade(p) :
      ...
      try:
        ...
        logger.warn('! Package "%s" (version %s) does not have "pool" in the uri %s \n' % (p.name, candidate_version, deb_uri))
        continue
  ...
  if DEB_FORMAT == 'deb' and a < c:
    logger.warn('**'+_('Very low disk space, need %(need)d kB have %(have)d kB'),
          {'need':c,'have':a})
