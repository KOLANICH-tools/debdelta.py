...
if len(sys.argv) <= 1:
    ...
    config.read(
        ["/etc/debdelta/sources.conf", os.path.expanduser("~/.debdelta/sources.conf")]
    )
    ...
    for a in sys.argv[1:]:
        ...
        for origin in p.candidate.origins:
            ...
            if delta_uri_base is None:
                print(
                    "  Sorry, sources.conf does not provide a server for this archive"
                )
                continue

            if installed_version == candidate_version:
                print(
                    "  Sorry, this package is already at its newest version for this archive"
                )
                continue
